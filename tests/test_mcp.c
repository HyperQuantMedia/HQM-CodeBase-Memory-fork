...
TEST(mcp_tools_list) {
    ...
    ASSERT_NOT_NULL(json);
    /* Every tool this fork serves: upstream's fifteen — including their targeted
     * coverage gate — plus our own ingest_overlay, so sixteen. Names, not a count:
     * an assertion on the number would pass while the wrong tool was missing. */
...
TEST(mcp_tools_have_behavior_annotations) {
    struct {
        ...
        /* This fork's own tool. Its absence here was not a test gap: the assertion
         * below compares the served list's length against this array, so a missing
         * entry is how we found that ingest_overlay had no TOOL_ANNOTATIONS row and
         * was being served with openWorldHint true. Idempotent unlike
         * ingest_traces, because a namespace is replaced rather than appended to. */
