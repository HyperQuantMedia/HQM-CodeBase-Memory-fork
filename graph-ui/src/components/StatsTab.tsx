  ...
  return (
    <div className="group relative inline-flex items-center">
      ...
      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 hidden group-hover:block z-20 pointer-events-none">
        <div className="bg-panel border border-border/50 rounded-lg px-3 py-2 text-[11px] whitespace-nowrap shadow-xl">
          <p className="font-medium" style={{ color: dotColor }}>{label}</p>
          {info && <p className="text-ink-dim text-[10px] mt-0.5">{info}</p>}
        </div>
      </div>
    </div>
  );
    ...
    <>
      <button
        ...
        className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-all ${
          hasAdr
            ...
            : "bg-surface-1 text-ink-dim hover:text-ink-soft hover:bg-surface-2"
        }`}
      >
        {hasAdr ? "ADR" : "+ ADR"}
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setOpen(false)}>
          ...
          <div className="relative bg-popover border border-border/40 rounded-2xl p-6 w-full max-w-2xl shadow-2xl max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-[15px] font-semibold text-foreground/90">{t.adr.title}</h3>
                <p className="text-[11px] text-ink-dim font-mono mt-0.5">{project}</p>
              </div>
              <button onClick={() => setOpen(false)} className="text-ink-faint hover:text-ink-soft text-[16px] p-1">×</button>
            </div>
            {updatedAt && (
              <p className="text-[10px] text-ink-faint mb-3">{t.adr.lastUpdated}: {updatedAt}</p>
            )}
            <textarea
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder={"# Architecture Decision Record\n\n## Context\n...\n\n## Decision\n...\n\n## Consequences\n..."}
              className="flex-1 min-h-[300px] bg-surface-1 border border-hairline rounded-xl px-4 py-3 text-[12px] text-foreground font-mono placeholder:text-ink-faint outline-none focus:border-primary/30 resize-none leading-relaxed"
            />
            <div className="flex justify-end gap-2 mt-4">
              ...
              {/* Ink roles are ours (`text-foreground/40` and `bg-white/[0.04]`
                  compile to a real alpha and cannot follow the light stage), but the
                  arrow function is theirs and it is a genuine fix: `save` now takes
                  `nextContent = content`, so `onClick={save}` hands it the React
                  MouseEvent and posts that as the ADR body. Keeping "ours" wholesale
                  here would have reintroduced the bug. */}
              <button onClick={() => setOpen(false)} className="px-4 py-2 rounded-lg text-[12px] text-ink-soft hover:bg-surface-2 font-medium transition-all">{t.common.cancel}</button>
...
function CreateIndexModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      <div className="absolute inset-0 bg-scrim backdrop-blur-sm" />
      <div className="relative bg-popover border border-border/40 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col overflow-hidden" style={{ height: "min(82vh, 680px)" }} onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="px-5 pt-5 pb-3 shrink-0">
          <h3 className="text-[15px] font-semibold text-foreground/90 mb-1">{t.index.selectRepositoryFolder}</h3>
          <p className="text-[12px] text-ink-dim">{t.index.instructions}</p>
        </div>

        <div className="px-5 pb-3 grid grid-cols-[1fr_220px] gap-3 shrink-0">
          <label className="block">
            <span className="block text-[10px] uppercase tracking-widest text-ink-dim mb-1">{t.index.repositoryPath}</span>
            <input
              ...
              className="w-full bg-surface-2 border border-hairline rounded-lg px-3 py-2 text-[12px] text-foreground font-mono outline-none focus:border-primary/40"
            />
          </label>
          <label className="block">
            <span className="block text-[10px] uppercase tracking-widest text-ink-dim mb-1">{t.index.projectName}</span>
            <input
              aria-label={t.index.projectName}
              value={projectName}
              placeholder={t.index.projectNamePlaceholder}
              onChange={(e) => setProjectName(e.target.value)}
              className="w-full bg-surface-2 border border-hairline rounded-lg px-3 py-2 text-[12px] text-foreground outline-none focus:border-primary/40 placeholder:text-ink-faint"
            />
            <span className="block text-[10px] text-ink-dim mt-1">{t.index.projectNameHelp}</span>
          </label>
        </div>
        ...
        <div className="px-5 pb-3 flex items-center gap-2 shrink-0">
          <input
            ...
            className="flex-1 bg-surface-2 border border-hairline rounded-lg px-3 py-2 text-[12px] text-foreground outline-none focus:border-primary/40 placeholder:text-ink-faint"
          />
          <div className="flex items-center gap-1">
            {displayRoots.map((root) => (
              <button
                ...
                onClick={() => browse(root)}
                className="px-2.5 py-2 rounded-lg bg-surface-2 hover:bg-surface-3 text-[11px] text-ink-soft font-mono transition-all"
              >
                {root}
              </button>
            ))}
        ...
        <div className="px-5 py-2 border-y border-border/20 flex items-center gap-0.5 overflow-x-auto text-[11px] shrink-0">
          ...
          {segments.map((seg, i) => (
            <span key={i} className="flex items-center gap-0.5 shrink-0">
              {(i > 0 || !isWinPath) && <span className="text-ink-faint">/</span>}
              <button
          ...
          <div className="px-2 py-1">
            ...
            {currentPath !== "/" && (
              <button
                ...
                className="flex items-center gap-2 w-full text-left px-3 py-2 rounded-lg hover:bg-surface-2 text-[12px] text-ink-soft transition-colors"
              >
            ...
            {loading ? (
              <p className="text-ink-faint text-[12px] text-center py-8">{t.common.loading}</p>
            ) : filteredDirs.length === 0 ? (
              <p className="text-ink-faint text-[12px] text-center py-8">{t.index.noSubdirectories}</p>
            ) : (
              filteredDirs.map((d, i) => (
                ...
                >
                  ...
                  >
                    <span className="text-ink-faint group-hover:text-ink-soft">/</span>
        ...
        <div className="px-5 py-4 border-t border-border/20 shrink-0">
          ...
          <div className="flex items-center justify-between">
            <p className="text-[11px] text-ink-dim font-mono truncate max-w-[250px]">{currentPath}</p>
            <div className="flex gap-2 shrink-0">
              <button onClick={onClose} className="px-3 py-2 rounded-lg text-[12px] text-ink-soft hover:bg-surface-2 font-medium transition-all">{t.common.cancel}</button>
      ...
      {active.map((j) => (
        <div key={j.slot} className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-ink-dim font-mono">{j.path}</p>
          </div>
        </div>
      ))}
      {errors.map((j) => (
      ...
      <div className="p-8 max-w-3xl mx-auto">
        {projects.length > 0 && (
          <div className="flex gap-4 mb-8">
            {[
              ...
              <div key={s.label} className="flex-1 rounded-xl border border-border/30 bg-surface-1 p-4">
                <p className="text-[10px] text-ink-dim uppercase tracking-widest mb-1">{s.label}</p>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={refresh} disabled={loading} className="px-3 py-1.5 rounded-lg bg-surface-2 hover:bg-surface-3 text-[12px] text-ink-soft font-medium transition-all disabled:opacity-30">{loading ? "..." : t.common.refresh}</button>
          </div>
        </div>
        ...
        {!loading && projects.length === 0 && !error && (
          <div className="text-center py-20">
            <p className="text-ink-dim text-[13px] mb-2">{t.projects.noIndexedProjects}</p>
        ...
        <div className="space-y-3">
          {projects.map((p) => {
            ...
            return (
              <div key={p.project.name} className="rounded-xl border border-border/30 bg-surface-1 hover:bg-surface-2 transition-all p-5">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="min-w-0 flex items-start gap-2.5">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-ink-faint font-mono truncate">{p.project.root_path}</p>
                    </div>
                  </div>
                ...
                {p.schema && (
                  <>
                    <div className="flex gap-6 text-[12px] text-ink-dim mb-3">
                      <span><strong className="text-ink-soft tabular-nums">{totalNodes.toLocaleString()}</strong> {t.projects.nodes}</span>
                      <span><strong className="text-ink-soft tabular-nums">{totalEdges.toLocaleString()}</strong> {t.projects.edges}</span>
                    </div>
