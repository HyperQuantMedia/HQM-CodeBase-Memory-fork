...
char *cbm_mcp_server_handle(cbm_mcp_server_t *srv, const char *line) {
    ...
    if (strcmp(req.method, "initialize") == 0) {
        ...
        /* No update check. It fetched
         * api.github.com/repos/DeusData/…/releases/latest through curl on every
         * session start — an outbound request compiled into every shipped binary,
         * pointed at ANOTHER organisation's releases, which is wrong for a fork on
         * its own merits regardless of the network cost. Removed with its thread,
         * its notice-injection and its four struct fields; the composition gate
         * asserts the URL is absent from the built artifact. */
        detect_session(srv);
