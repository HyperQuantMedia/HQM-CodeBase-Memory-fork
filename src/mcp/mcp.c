...
static const tool_annotation_def_t TOOL_ANNOTATIONS[] = {
    ...
    /* ingest_overlay had no entry here until 2026-07-31, so it fell through to
     * mcp_add_tool_def's defaults — destructiveHint true and, worse,
     * openWorldHint TRUE, which contradicts the comment above this table and told
     * every agent the tool might reach outside the local domain. It does not: the
     * manifest is read from the project root or passed inline.
     *
     * destructive: true — re-ingest replaces everything under the manifest's
     * declared namespace, so a second call with a smaller manifest removes nodes.
     * idempotent: true — and this is the difference from ingest_traces, which
     * accumulates trace counts and therefore is not: the same overlay manifest
     * ingested twice leaves the graph in the same state, because the namespace is
     * replaced rather than appended to. */
    {"ingest_overlay", false, true, true, false},
};
