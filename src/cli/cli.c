...
static const char skill_content[] =
    ...
    "direction=\"inbound\")`\n"
    "\n"
    /* 16 = upstream's 15 plus this fork's `ingest_overlay`. The number is kept in
     * step with the list below by hand, and the list is the authority — a count
     * that disagrees with what follows it is worse than no count. Note upstream's
     * own #1297 disputes their figure (it argues `semantic_query` is not a tool and
     * the real count is 14), so do not treat 15 as settled on their side; what is
     * settled here is that the sixteen names below are the sixteen this binary
     * serves. */
    "## 16 MCP Tools\n"
    "`index_repository`, `index_status`, `list_projects`, `delete_project`,\n"
    "`search_graph`, `search_code`, `trace_path`, `detect_changes`,\n"
    "`query_graph`, `get_graph_schema`, `get_code_snippet`, `get_architecture`,\n"
    "`check_index_coverage`, `manage_adr`, `ingest_traces`, `ingest_overlay`\n"
    "\n"
    "## Edge Types\n"
