...
static void handle_ui_config(cbm_http_conn_t *c, const cbm_http_req_t *req) {
    ...
    /* Language only. The upstream version of this handler also served an
     * `upstream_issues_url` pointing at its own GitHub issue tracker, which the
     * missed-coverage callout turned into a prefilled "report this indexing gap"
     * link. That key is removed rather than repointed: no end-user surface here
     * routes a user into anyone's issue tracker, and there is nothing to repoint it
     * at that would not be the same disclosure in the other direction.
     *
     * Dropping it also removes the only external URL this endpoint carried, which is
     * the outcome the audit rule behind it was after in the first place. */
...
/* The PID-authorization helper that used to live here is gone with the endpoint it
 * guarded.
 *
 * It existed because our Windows kill path was real while the POSIX guard —
 * "only PIDs this server itself forked" — sat inside `#ifndef _WIN32` and was
 * compiled out, leaving a loopback endpoint that could terminate any process on
 * the machine by PID. Nothing exercised it while the process list was always
 * empty; making the list real put a kill button beside every row.
 *
 * Upstream reached the stronger conclusion in the same window and deleted the
 * route outright, with a test that now asserts it 404s
 * (`ui_server_process_kill_route_is_unavailable`): a PID is not a cancellation
 * capability, and daemon-owned jobs are cancelled through opaque owner-bound
 * handles instead. So the guard is not needed — there is nothing left to
 * authorize, which is a better outcome than a correct check on a capability that
 * should not exist. `win_enumerate_cbm_processes` stays: /api/processes still
 * lists, and listing is all it does. */
